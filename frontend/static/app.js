...
async function loadStrategies() {
  ...
  strategyEl.innerHTML = data.map((s) => `<option value="${s.key}">${s.name} (${s.complexity})</option>`).join("");
}
...
async function loadNews(query) {
  ...
  newsList.innerHTML = data.items
    .map((x) => `<div class="news-item"><a href="${x.link}" target="_blank" rel="noopener noreferrer">${x.title}</a><div>${x.source || "News"} | ${x.pub_date || ""}</div></div>`)
    .join("");
}
...
function renderTrades(trades) {
  tradesBody.innerHTML = trades
    .slice(-50)
    .reverse()
    .map((t) => `<tr><td>${new Date(t.time).toLocaleString()}</td><td>${t.price.toFixed(2)}</td><td>${t.position.toFixed(3)}</td></tr>`)
    .join("");
}
